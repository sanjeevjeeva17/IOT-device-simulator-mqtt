...
function publishAsync(authToken, messageCount, numMessages) {
  ...
  const payload = JSON.stringify(device_data);
...
function getConfig(authToken, version) {
  ...
  console.log(JSON.stringify(request.RetryStrategies) + 'what is this');
  request(options, (error, response, body) => {
    if (error) {
      ...
      console.log('Received config response', JSON.stringify(response));
    }
  });
}
